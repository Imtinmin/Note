...
public function getip2(){
    ...
    if (!empty($_FILES['screen']['name'])) {
        if (preg_match('/^image\//i', $_FILES['screen']['type'])) {
            $fileinfo = pathinfo($_FILES['screen']['name']);
            $filename = substr(md5($_FILES['screen']['name']), 0, 8) . ".{$fileinfo['extension']}";
            if (!file_exists("{$user_upload_dir}/public/screen")) {
                mkdir("{$user_upload_dir}/public/screen");
            }
            if (file_exists($filename)) {
                unlink($filename);
            }
            if (move_uploaded_file($_FILES['screen']['tmp_name'], $filename)) {
                $data['screen'] = str_replace(SITE_DIR . '/', '', $filename);
                $data['screen'] = "http://{$site_url}" . SITE_PATH . "/{$data['screen']}";
            }
        }
    }
}
