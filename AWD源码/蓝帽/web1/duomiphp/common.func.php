...
{
	return "<p> <strong><a href=\"http://www.xxx.com\" title=\"".$GLOBALS['cfg_softname']."\" target=\"_blank\">".$GLOBALS['cfg_soft_enname']."</a></strong> <em>".$GLOBALS['cfg_version']."</em></p>";
}
...
function ShowMsg($msg,$gourl,$onlymsg=0,$limittime=0,$extraJs='')
{
	...
	else
	{
      ...
      function JumpUrl(){
		...
		$rmsg = $func;
		$rmsg .= "document.write(\"<br /><div style='width:450px;border:1px solid #DDDDDD;margin-top:200px;box-shadow: 1px 1px 5px #CDCDCD;border-radius:5px;'>";
		
        
        $rmsg .= "<div style='border-bottom:1px solid #DDDDDD;padding:10px;font-size:14px;color:#000;background:#F5F5F5 url({$GLOBALS['cfg_phpurl']}images/wbg.gif)';'><b>系统提示</b></div>\");\r\n";
		...
		if($onlymsg==0)
		{
			...
			{
				$rmsg .= "<br /><a style='font-size:11px;color:#DBEAF9; text-decoration:none;color:#000'  href='{$gourl}'>如果浏览器没反应，请点击这里</a><div style='border-top:1px solid #DDDDDD;height:65px;background:#F5F5F5'></div>";
...
function Html2Text($str,$r=0)
{
	...
	{
		require_once(duomi_INC."/inc/inc_fun_funString.php");
	}
	if($r==0)
	{
...
function CreateDir($spath)
{
	...
	{
		require_once(duomi_INC.'/inc/inc_fun_funAdmin.php');
	}
...
{
	$textsegment=duomi_DATA.'/admin/textsegment.xml';
...
function makePageNumber($currentPage,$pagelistLen,$totalPages,$linkType,$currentTypeId=0,$vid=0){
	...
	for($pagenumber=$beginPage;$pagenumber<=$endPage;$pagenumber++){
		...
		if($pagenumber==$currentPage){
			...
			break;
			case "videolist":
			global $order,$type,$keyword,$v_state,$v_commend,$repeat,$topic,$playfrom,$etype,$empty,$rlen,$allrepeat,$v_recycled,$jqtype,$area,$year,$yuyan,$letter,$commend,$ver,$tid;
...
function getTopicPageLink($topicId,$page)
{
	...
	if($GLOBALS['cfg_runmode']=='0'){
		$row=$dsql->GetOne("select enname from duomi_topic where id=".$topicId);
...
function getPlayerKindsArray(){
	...
	$m_file = duomi_DATA."/admin/playerKinds.xml";
...
{
	$temp=array();
	$m_file = duomi_DATA."/admin/downKinds.xml";
...
function setCache($p_cacheName,$sql="",$arr=""){
	...
	$cacheFile=duomi_ROOT.'/data/cache/'.$cfg_cachemark.$p_cacheName.'.inc';
...
function lib_hotwords($num=5)
{
	...
	$dsql->SetQuery("Select keyword From `duomi_search_keywords` where lasttime>$mintime And length(keyword)<$maxlength order by count desc limit 0,$num");
	$dsql->Execute('hw');
	$hotword = '';
	$i=1;
...
function front_member()
{
	...
	$member = '<span id="duomicms_member"></span><script>member()</script>';
	}
	return $member;
}
...
function getDataCount($countType){
	...
	switch ($countType) {
		case "all":
		$row = $dsql->GetOne("select count(*) as dd From `duomi_data`");
		$DataCount=$row['dd'];
		break;
		case "day":
		$row = $dsql->GetOne("select count(*) as dd From `duomi_data` $whereStr");
		$DataCount=$row['dd'];
		break;
	}
	return $DataCount;
...
function getTypeLists($type=0)
{
	...
	$sql="select tid,upid,tname,tenname,torder,templist,templist_1,keyword,description,ishidden,unionid,tptype,title,-1 as tcount from duomi_type where tptype = '$type' order by torder asc";
	$rows=array();
	$dsql->SetQuery($sql);
	$dsql->Execute('al');
...
{
	global $dsql,$cfg_iscache;
	$sql="select tid,tname,ishidden,-1 as tcount from duomi_jqtype";
	$rows=array();
	$dsql->SetQuery($sql);
	$dsql->Execute('al');
	while($rowr=$dsql->GetObject('al'))
...
function getNumPerTypeOfNews($id)
{
	...
	$sql = "SELECT count(n_id) as dd FROM `duomi_news` where tid in (".$ids.")";
	$row=$dsql->GetOne($sql);
	if(is_array($row))
	{
		$num=$row['dd'];
...
function getNumPerType($id)
{
	...
	$row=$dsql->GetOne("SELECT count(v_id) as dd FROM `duomi_data` where tid in (".$ids.")");
	if(is_array($row))
	{
		$num=$row['dd'];
	}
...
function getNumPerjqType($id)
{
	...
	$tlist = getTypeListsOnCache();
	$ids = getTypeIdOnCache($id);
	$row=$dsql->GetOne("SELECT count(v_id) as dd FROM `duomi_data` where v_jq like '%$ids%'");
	if(is_array($row))
	{
		$num=$row['dd'];
	}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_title FROM duomi_news WHERE n_id=$Id");
	return $row['n_title'];
}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_entitle FROM duomi_news WHERE n_id=$Id");
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_name,v_enname FROM duomi_data WHERE v_id=$videoId");
	if($row['v_enname']!="")
	{
		return $row['v_enname'];
	}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_name FROM duomi_data WHERE v_id=$videoId");
	return $row['v_name'];
}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_addtime FROM duomi_data WHERE v_id=$videoId");
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_addtime FROM duomi_news WHERE n_id=$Id");
...
function chkFileCache($cacheName)
{
	...
	$cacheFile=duomi_ROOT.'/data/cache/'.$cfg_cachemark.$cacheName.'.inc';
	$mintime = time() - $cfg_cachetime*60;
	if(!file_exists($cacheFile) || ( file_exists($cacheFile) && ($mintime > filemtime($cacheFile)))){
		return false;
	}else{
...
{
	global $dsql;
	$sql="select name from duomi_topic where id=".$id;
	$row=$dsql->GetOne($sql);
	$tpname=$row['name'];
	return $tpname;
}
...
{
	$labelRule = buildregx("{duomicms:".$ptype."list(.*?)size=([0-9]+)(.*?)}","is");
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)size=([0-9]+)(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)lang=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)year=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)area=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)order=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)time=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)commend=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)type=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)letter=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)state=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)maxpage=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{duomicms:customvideolist(.*?)jq=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$str=str_replace("{duomicms:currenttypeid}",$currentTypeId,$str);
	return $str;
}
...
{
	global $dsql;
	$rowc=$dsql->GetOne("select vod as dd from duomi_topic where id=".$topicId);
	$topicvod = $rowc['dd'];
...
{
	global $dsql;
	$cachefile = duomi_DATA.'/cron.cache.php';
	$row = $dsql->GetOne("SELECT nextrun,filename FROM duomi_crons WHERE available>'0' AND nextrun>'0' ORDER BY nextrun");
	...
	$cachedata = "<?php\r\n//sduomi2.0 cache file\r\n//Created on ".MyDate('Y-m-d H:i:s',time())."\r\n\r\nif(!defined('duomi_INC')) exit('Access Denied');\r\n\r\n".$contents."\r\n\r\n?>";
...
function RWCache($name,$value='') {
    static $_cache = array();
    $filename   =   duomi_DATA.'/cache/'.$name.'.php';
    if('' !== $value) {
        if(is_null($value)) {
            // 删除缓存
...
{
	@session_start();
	$usergroup=$_SESSION["duomi_user_group"];
...
{
	global $dsql;
	$dsql->SetQuery("select * from duomi_member_group");
...
{
	global $cfg_banIPS;
	$iplisttxt=duomi_DATA."/admin/iplist.txt";
