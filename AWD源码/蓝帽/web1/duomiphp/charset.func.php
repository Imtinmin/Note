...
function utf82gb($utfstr)
{
	...
	if(empty($UC2GBTABLE))
	{
		$filename = duomi_INC."/data/gb2312-utf8.dat";
		$fp = fopen($filename,"r");
		while($l = fgets($fp,15))
		{
...
function big52gb($Text)
{
	...
	if(empty($BIG5_DATA))
	{
		$filename = duomi_INC."/data/big5-gb.dat";
...
function gb2big5($Text)
{
	...
	if(empty($GB_DATA))
	{
		$filename = duomi_INC."/data/gb-big5.dat";
		$fp = fopen($filename, "rb");
...
function UnicodeUrl2Gbk($str)
{
	...
	{
		$fp = fopen(duomi_INC.'/data/gbk-unicode.dat','rb');
		while(!feof($fp))
		{
