...
$m_file = duomi_ROOT."/player/play.js";
$playerKindsfile = duomi_DATA."/admin/playerKinds.xml";
if($action=="edit")
{
	...
	$player=preg_replace("/mplayerw='(\d+)';/is","mplayerw='".$mplayerwidth."';",$player);
	$player=preg_replace("/mplayerh='(\d+)';/is","mplayerh='".$mplayerheight."';",$player);
	$player=preg_replace("/player_jxurl=(.*?)\";/is","player_jxurl=\"".$playerjxurl."\";",$player);
	...
	$player=preg_replace("/duomi_Player_File=(.*?)\";/is","duomi_Player_File=\"$playerset\";",$player);
...
elseif($action=="save")
{
	...
	$sql = "select * from `duomi_playdata` ";
...
{
	include(duomi_ADMIN.'/html/admin_player.htm');
	exit();
}
...
else
{
	...
	$playerjxurl=getrulevalue($player,"player_jxurl=\"","\";");
	...
	$playerset=getrulevalue($player,"duomi_Player_File=\"","\";");
