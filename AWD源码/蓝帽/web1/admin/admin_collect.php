...
elseif($action=="addsave")
{
	...
	$inquery = " INSERT INTO `duomi_co_config`(`cname`,`getlistnum`,`getconnum`)VALUES ('$cname','$getlistnum','$getconnum')";
...
{/duomicms:listrule}\r\n";
		...
		if(!is_array($row)){
			$inquery = " INSERT INTO `duomi_co_type`(`cid`,`tname`,`siteurl`,`getherday`,`coding`,`sock`,`playfrom`,`autocls`,`classid`,`addtime`,`listconfig`)VALUES ('$id','$itemname','$siteurl','$getherday','$coding','$sock','$playfrom','$autocls','$classid','".time()."','$listconfig')";
			$rs = $dsql->ExecuteNoneQuery($inquery);
			if(!$rs)
			{
				ShowMsg("保存信息时出现错误！".$dsql->GetError(),"-1");
		...
		include(duomi_ADMIN.'/html/admin_collect_ruleadd3.htm');
		exit();
	}elseif($step==4){
		if(!empty($fields)) $fds = implode(',',$fields); else $fds="";
		if(empty($serveron)) $serveron=2;
		$itemconfig = "{duomicms:itemconfig fields=\"$fds\" splay=\"$splay\" serveron=\"$serveron\" playgetsrc=\"$playgetsrc\" getpart=\"$getpart\" intodatabase=\"$intodatabase\"}";
...
{/duomicms:listrule}\r\n";
		...
		$listconfig = urldecode($listconfig);
			...
			$dsql->ExecuteNoneQuery($upquery);
		include(duomi_ADMIN.'/html/admin_collect_ruleedit3.htm');
		exit();
	}elseif($step==4){
		if(!empty($fields)) $fds = implode(',',$fields); else $fds="";
		if(empty($serveron)) $serveron=2;
...
//	print_r($_POST); die();
	...
	{
		if(!$uesMode)
		$dsql->ExecNoneQuery("insert into duomi_co_filters(Name,rColumn,uesMode,sFind,sReplace,Flag) values ('".$name."',$rColumn,0,'".$sFind."','".$sReplace."',$Flag)");
		else 
		$dsql->ExecNoneQuery("insert into duomi_co_filters(Name,rColumn,uesMode,sFind,sReplace,Flag,sStart,sEnd) values('".$name."',$rColumn,1,'".$sFind."','".$sReplace."',$Flag,'".$sStart."','".$sEnd."')");
...
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_config where tid=".$id);
...
elseif($action=="delall")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_config where cid in (".$ids.")");
...
elseif($action=="exportrule")
{
	...
	{
		$tid=$rowt->tid;
		$rowr=$dsql->GetOne("select tid,cid,tname,siteurl,getherday,playfrom,autocls,classid,coding,sock,addtime,cjtime,listconfig,itemconfig,isok from duomi_co_type where tid='$tid'");
		$rule['type'][$tid]=$rowr;
	}
		//print_r($rule);
...
elseif($action=="copyrulesingle")
{
	...
	$tname=$tname."[复制]";
	$addtime=time();
	$inquery = " INSERT INTO `duomi_co_type`(`cid`,`tname`,`siteurl`,`getherday`,`playfrom`,`autocls`,`classid`,`coding`,`sock`,`addtime`,`listconfig`,`itemconfig`,`isok`)
...
elseif($action=="customernewcls")
{
	...
	$in_query = "insert into `duomi_co_cls`(clsname,sysclsid) Values('$clsname','$tid')";
...
{
	include(duomi_ADMIN.'/html/admin_co_tempdatabase.htm');
	exit();
}
elseif($action=="editVideo")
{
	...
	//读取影片信息
	$query = "select * from duomi_co_data where v_id='$id' ";
	...
	include(duomi_ADMIN.'/html/admin_co_editvideo.htm');
	exit();
}
elseif($action=="saveVideo")
{
	...
	switch (trim($acttype)) 
	{
		case "edit":
			...
			$updateSql = "update duomi_co_data set ".$updateSql." where v_id=".$v_id;
...
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_data where v_id=".$id);
...
elseif($action=="delallTempData")
{
	if($do=="all")
	{
		$dsql->ExecuteNoneQuery("delete from duomi_co_data");
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_data where v_id in(".$ids.")");
...
elseif($action=="import")
{
	...
	if(!empty($smode)){
		...
		{
			$csqlStr="select count(*) as dd from `duomi_co_data` ".$wherestr;
		...
		$sql="select v_id from `duomi_co_data` ".$wherestr." limit $start,$numPerPage";
		$dsql->SetQuery($sql);
		...
		for ($i=$getpagebegin; $i<=$getpageend; $i++)
		{
			...
			if (!empty($links))
			{
				for ($j=0;$j<count($links);$j++)
				{
					...
					if(is_array($rowt)){
						$dsql->ExecuteNoneQuery("update `duomi_co_url` set succ='0',err='0',pic='$pic' where uid=".$rowt['uid']);
						echo "{$echo_id}. $url\t<font color=red>已存在，更新采集数据</font><br>";
					}else{
						$sql="insert into `duomi_co_url`(cid,tid,url,pic) values ('$cid','$id','$url','$pic')";
...
{
	$publishareatxt=duomi_DATA."/admin/publishyuyan.txt";
	$publisharea = array();
	if(filesize($publishareatxt)>0)
	{
		$publisharea = file($publishareatxt);
...
function import2Base($idsArray,$vtype)
{
	...
	if(count($idsArray)>0)
	{
		...
		$sql="SELECT v_id,v_name,v_letter,tid,v_state,v_pic,v_director,v_actor,v_des,v_playdata,v_publishyear,v_publisharea,v_hit,v_note,v_tags,v_lang FROM duomi_co_data WHERE v_id IN (".$ids.")";
		$dsql->SetQuery($sql);
		...
		while($row=$dsql->GetAssoc('import_list'))
		{
			...
			if($row['tid']!=''){
				...
				$dsql->ExecuteNoneQuery("update `duomi_co_data` set v_inbase='1',tid='".$row['tid']."' where v_id=".$row['v_id']);
			}else{
