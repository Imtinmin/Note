...
require_once(duomi_INC."/collection.func.php");
CheckPurview();
if(empty($action))
{
	$action = '';
...
if($action=="add")
{
	include(duomi_ADMIN.'/html/admin_collect_add.htm');
	exit();
}
elseif($action=="addsave")
{
	...
	$inquery = " INSERT INTO `duomi_co_config`(`cname`,`getlistnum`,`getconnum`,`cotype`)VALUES ('$cname','$getlistnum','$getconnum','1')";
...
if($action=="addrule")
{
	if($step==2){
		...
		$listconfig = "{duomicms:listrule cid=\"$id\" tname=\"$itemname\" isupdate=\"$isupdate\" getherday=\"$getherday\" siteurl=\"$siteurl\" playfrom=\"$playfrom\" autocls=\"$autocls\" classid=\"$classid\" inithit=\"$inithit\" pageset=\"$pageset\" pageurl0=\"$pageurl0\" pageurl1=\"$pageurl1\" istart=\"$istart\" iend=\"$iend\" reverse=\"$reverse\"}";
		include(duomi_ADMIN.'/html/admin_collect_ruleadd2.htm');
		exit();
	}elseif($step==test){
		$listconfig = urldecode($listconfig);
		$listconfig.="
			{duomicms:pageurl2}$pageurl2{/duomicms:pageurl2}
			{duomicms:lista}$lista{/duomicms:lista}
			{duomicms:listb}$listb{/duomicms:listb}
			{duomicms:mlinka}$mlinka{/duomicms:mlinka}
			{duomicms:mlinkb}$mlinkb{/duomicms:mlinkb}
			{duomicms:picmode}$picmode{/duomicms:picmode}
			{duomicms:pica}$pica{/duomicms:pica}
			{duomicms:picb}$picb{/duomicms:picb}
			{duomicms:pic_trim}$pic_trim{/duomicms:pic_trim}
{/duomicms:listrule}\r\n";
		$tmplistconfig = stripslashes($listconfig);
		$links=array();
		$links = Testlists($tmplistconfig,$coding,$sock);
		include(duomi_ADMIN.'/html/admin_collect_ruleadd2test.htm');
		exit();
	}elseif($step==3){
		$listconfig = urldecode($listconfig);
		$row = $dsql->GetOne("Select tid From `duomi_co_type` where isok=0 And tname like '$itemname' ");
		if(!is_array($row)){
			$inquery = " INSERT INTO `duomi_co_type`(`cid`,`tname`,`siteurl`,`getherday`,`coding`,`sock`,`playfrom`,`autocls`,`classid`,`addtime`,`listconfig`,`cotype`)VALUES ('$id','$itemname','$siteurl','$getherday','$coding','$sock','$playfrom','$autocls','$classid','".time()."','$listconfig','1')";
			$rs = $dsql->ExecuteNoneQuery($inquery);
			if(!$rs)
			{
				ShowMsg("保存信息时出现错误！".$dsql->GetError(),"-1");
			...
			$tid=$row['tid'];
			$upquery = "update `duomi_co_type` set `cid`='$id',`tname`='$itemname',`siteurl`='$siteurl',`getherday`='$getherday',`coding`='$coding',`sock`='$sock',`playfrom`='$playfrom',`autocls`='$autocls',`classid`='$classid',`addtime`='".time()."',`listconfig`='$listconfig' where tid='$tid'";
			$dsql->ExecuteNoneQuery($upquery);
		}
		include(duomi_ADMIN.'/html/admin_collectnews_ruleadd3.htm');
		exit();
	}elseif($step==4){
		if(!empty($fields)) $fds = implode(',',$fields); else $fds="";
		$itemconfig = "{duomicms:itemconfig fields=\"$fds\"}";
		foreach($item as $field){
			$stra = $GLOBALS[$field."a"];
			$strb = $GLOBALS[$field."b"];
			$trimstr = $GLOBALS[$field."_trim"];
			$itemconfig .="
			{duomicms:".$field."a}$stra{/duomicms:".$field."a}";
			$itemconfig .="
			{duomicms:".$field."b}$strb{/duomicms:".$field."b}";
			$itemconfig .="
			{duomicms:".$field."_trim}$trimstr{/duomicms:".$field."_trim}";
		}
		$itemconfig .="
		{/duomicms:itemconfig}\r\n";
		$upquery = "update `duomi_co_type` set `itemconfig`='$itemconfig' where tid='$tid'";
		$dsql->ExecuteNoneQuery($upquery);
		$cofrom=1;
		include(duomi_ADMIN.'/html/admin_collect_ruleadd4.htm');
		exit();
	}elseif($step==5){
		$dsql->ExecuteNoneQuery("Update `duomi_co_type` set isok='1' where tid='$tid' ");
		ShowMsg("添加采集规则成功！","admin_collect_news.php");
		exit();
	}else{
		$cofrom=1;
		include(duomi_ADMIN.'/html/admin_collect_ruleadd.htm');
		exit();
	}
}
elseif($action=="editrulesingle")
{
	$query = "select * from `duomi_co_type` where tid='$id' ";
	...
	$labelRule = buildregx("{duomicms:listrule(.*?)}(.*?){/duomicms:listrule}","is");
	preg_match_all($labelRule,$row[listconfig],$ar);
	$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][0]));
	$loopstr=$ar[2][0];
	$atd=parduomittr($attrStr);
	if($step==2){
		$listconfig = "{duomicms:listrule cid=\"$id\" tname=\"$itemname\" isupdate=\"$isupdate\" getherday=\"$getherday\" siteurl=\"$siteurl\" playfrom=\"$playfrom\" autocls=\"$autocls\" classid=\"$classid\" inithit=\"$inithit\" pageset=\"$pageset\" pageurl0=\"$pageurl0\" pageurl1=\"$pageurl1\" istart=\"$istart\" iend=\"$iend\" reverse=\"$reverse\"}";
		include(duomi_ADMIN.'/html/admin_collect_ruleedit2.htm');
		exit();
	}elseif($step==test){
		$listconfig = urldecode($listconfig);
		$listconfig.="
...
{/duomicms:listrule}\r\n";
		...
		$labelitemRule = buildregx("{duomicms:itemconfig(.*?)}(.*?){/duomicms:itemconfig}","is");
		preg_match_all($labelitemRule,str_replace('&nbsp;','#n#',$row[itemconfig]),$itemar);
		$itemattrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$itemar[1][0]));
		$itemloopstr=$itemar[2][0];
		$atr=parduomittr($itemattrStr);
		$flds=$atr["fields"];
		$listconfig = urldecode($listconfig);
			$upquery = "update `duomi_co_type` set `tname`='$itemname',`siteurl`='$siteurl',`getherday`='$getherday',`coding`='$coding',`sock`='$sock',`playfrom`='$playfrom',`autocls`='$autocls',`classid`='$classid',`addtime`='".time()."',`listconfig`='$listconfig' where tid='$id'";
			$dsql->ExecuteNoneQuery($upquery);
		include(duomi_ADMIN.'/html/admin_collectnews_ruleedit3.htm');
		exit();
	}elseif($step==4){
		if(!empty($fields)) $fds = implode(',',$fields); else $fds="";
		$itemconfig = "{duomicms:itemconfig fields=\"$fds\" splay=\"$splay\"}";
		foreach($item as $field){
			$stra = $GLOBALS[$field."a"];
			$strb = $GLOBALS[$field."b"];
			$trimstr = $GLOBALS[$field."_trim"];
		...
		$upquery = "update `duomi_co_type` set `itemconfig`='$itemconfig' where tid='$id'";
		$dsql->ExecuteNoneQuery($upquery);
		$cofrom=1;
		include(duomi_ADMIN.'/html/admin_collect_ruleedit4.htm');
		exit();
	}elseif($step==5){
		$dsql->ExecuteNoneQuery("Update `duomi_co_type` set isok='1' where tid='$tid' ");
		...
		$cofrom=1;
		include(duomi_ADMIN.'/html/admin_collect_ruleedit.htm');
		exit();
	}
}
...
{
	$cofrom=1;
	include(duomi_ADMIN.'/html/admin_collect_filters.htm');
	exit();
}
elseif($action=="filtersadd")
{
	include(duomi_ADMIN.'/html/admin_collect_filtersadd.htm');
	exit();
}
elseif($action=="filtersedit")
{
	include(duomi_ADMIN.'/html/admin_collect_filtersedit.htm');
	exit();	
}
...
//	print_r($_POST); die();
	...
	{
		if(!$uesMode)
		$dsql->ExecNoneQuery("insert into duomi_co_filters(Name,rColumn,uesMode,sFind,sReplace,Flag,cotype) values ('".$name."',$rColumn,0,'".$sFind."','".$sReplace."',$Flag,'1')");
		else 
		$dsql->ExecNoneQuery("insert into duomi_co_filters(Name,rColumn,uesMode,sFind,sReplace,Flag,sStart,sEnd,cotype) values('".$name."',$rColumn,1,'".$sFind."','".$sReplace."',$Flag,'".$sStart."','".$sEnd."','1')");
...
//		exit();
	...
	{
		if(!$uesMode)
		$dsql->ExecNoneQuery("update duomi_co_filters set Name='".$name."',rColumn=".$rColumn.",uesMode=".$uesMode.",sFind='".$sFind."',sReplace='".$sReplace."',Flag=".$Flag." where ID=".$id );
		else 
		$dsql->ExecNoneQuery("update duomi_co_filters set Name='".$name."',rColumn=".$rColumn.",uesMode=".$uesMode.",sFind='".$sFind."',sReplace='".$sReplace."',Flag=".$Flag.",sStart='".$sStart."',sEnd='".$sEnd."' where ID=".$id );
...
{
	$sql = "delete from duomi_co_filters where ID=".$id;
...
elseif ($action=="filtersdelall")
{
	...
	$ida = rtrim($ida,",");
	$sql = "delete from duomi_co_filters where ID in ($ida)";
	$dsql ->ExecNoneQuery($sql);
	ShowMsg('删除成功！', 'admin_collect_news.php?action=filters');	
...
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_type where tid=".$id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_url where tid=".$id);
...
elseif($action=="delall")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_type where tid in(".$ids.")");
	$dsql->ExecuteNoneQuery("delete from duomi_co_url where tid in(".$ids.")");
	ShowMsg("规则删除成功","admin_collect_news.php");
	exit();
}
elseif($action=="customercls")
{
	...
	$cofrom=1;
	include(duomi_ADMIN.'/html/admin_collect_customercls.htm');
	exit();
}
...
{
	$sql = "select coding,sock,listconfig from  duomi_co_type where tid=$id";
	...
	$cofrom=1;
	include(duomi_ADMIN.'/html/admin_collect_getcontest.htm');
	exit();
}
elseif($action=="resetlist")
{
	$dsql->ExecuteNoneQuery("update duomi_co_url set succ='0' where cid='$id'");
	header("Location:admin_collect_news.php");
	die;
}
elseif($action=="reseterr")
{
	$dsql->ExecuteNoneQuery("update duomi_co_url set err='0' where cid='$id'");
	header("Location:admin_collect_news.php");
	die;
}
elseif($action=="clearlist")
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_url where cid='$id'");
	header("Location:admin_collect_news.php");
	die;
}
elseif($action=="editrule")
{
	$row=$dsql->GetOne("select * from duomi_co_config where cid='$id'");
	include(duomi_ADMIN.'/html/admin_collect_edit.htm');
	exit();
}
elseif($action=="editsave")
{
	...
	$dsql->ExecuteNoneQuery("update duomi_co_config set cname='$cname',getlistnum='$getlistnum',getconnum='$getconnum' where cid='$id'");
	header("Location:admin_collect_news.php");
	die;
}
elseif($action=="delrule")
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_config where cid='$id'");
	$dsql->ExecuteNoneQuery("delete from duomi_co_type where cid='$id'");
	$dsql->ExecuteNoneQuery("delete from duomi_co_url where cid='$id'");
	header("Location:admin_collect_news.php");
	die;
}
elseif($action=="exportrule")
{
	$row=$dsql->GetOne("select * from duomi_co_config where cid='$id'");
	$rule['config']=$row;
	$dsql->SetQuery("Select * From `duomi_co_type` where cid='$id' order by tid");
	$dsql->Execute('cotype_list');
	while($rowt=$dsql->GetObject('cotype_list'))
	{
		$tid=$rowt->tid;
		$rowr=$dsql->GetOne("select * from duomi_co_type where tid='$tid'");
		$rule['type'][$tid]=$rowr;
	}
	$export = serialize($rule);
	$export = "BASE64:".base64_encode($export).":END";
	unset($rule);
	include(duomi_ADMIN.'/html/admin_collect_exportrule.htm');
	exit();
}
elseif($action=="importrule")
{
	include(duomi_ADMIN.'/html/admin_collect_importrule.htm');
	exit();
}
...
{
	$row = $dsql->GetOne("Select * From `duomi_co_type` where tid='$id'");
	foreach($row as $k=>$v)
	{
		if(!isset($$k))
		{
	...
	$tname=$tname."[复制]";
	$addtime=time();
	$inquery = " INSERT INTO `duomi_co_type`(`cid`,`tname`,`siteurl`,`getherday`,`playfrom`,`autocls`,`classid`,`coding`,`sock`,`addtime`,`listconfig`,`itemconfig`,`isok`,`cotype`)
...
elseif($action=="list")
{
	include(duomi_ADMIN.'/html/admin_collect_list.htm');
	exit();
}
elseif($action=="customernewcls")
{
	...
	$in_query = "insert into `duomi_co_cls`(clsname,sysclsid,cotype) Values('$clsname','$tid','1')";
...
elseif($action=="customersavecls")
{
	...
	foreach($e_id as $id)
	{
		...
		$dsql->ExecuteNoneQuery("update duomi_co_cls set clsname='$clsname',sysclsid='$tid' where id=".$id);
	}
...
elseif($action=="customerdelallcls")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_cls where id in(".$ids.")");
	header("Location:?action=customercls");
	die;
}
elseif($action=="customerdelcls")
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_cls where id=".$id);
	header("Location:?action=customercls");
	die;
}
elseif($action=="tempdatabase")
{
	include(duomi_ADMIN.'/html/admin_co_newstempdatabase.htm');
	exit();
}
elseif($action=="editNews")
{
	...
	//读取影片信息
	$query = "select * from duomi_co_news where n_id='$id' ";
	...
	include(duomi_ADMIN.'/html/admin_co_editnews.htm');
	exit();
}
elseif($action=="saveVideo")
{
	...
	switch (trim($acttype)) 
	{
		case "edit":
			...
			$updateSql = "update duomi_co_news set ".$updateSql." where n_id=".$n_id;
...
{
	$dsql->ExecuteNoneQuery("delete from duomi_co_news where n_id=".$id);
...
elseif($action=="delallTempData")
{
	if($do=="all")
	{
		$dsql->ExecuteNoneQuery("delete from duomi_co_news");
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_co_news where n_id in(".$ids.")");
...
elseif($action=="import")
{
	...
	if(!empty($smode)){
		...
		$csqlStr="select count(*) as dd from `duomi_co_news` ".$wherestr;
		...
		$sql="select n_id from `duomi_co_news` ".$wherestr." limit $limitstart,$numPerPage";
		$dsql->SetQuery($sql);
		$dsql->Execute('duomi_co_news');
		while($row=$dsql->GetObject('duomi_co_news'))
		{
			$idsArray[]=$row->n_id;
		}
		if(count($idsArray)>0){
...
{
	$cofrom=1;
	include(duomi_ADMIN.'/html/admin_collect.htm');
	exit();
}
...
function getlistsbyid($id,$cid='',$action2='',$index='')
{
	...
	$row = $dsql->GetOne("Select t.coding,t.sock,t.playfrom,t.autocls,t.classid,t.getherday,t.listconfig,c.cid,c.getlistnum from `duomi_co_type` t left join `duomi_co_config` c on c.cid=t.cid where t.tid='$id'");
	...
	$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][0]));
	$loopstr=$ar[2][0];
	$attrDictionary=parduomittr($attrStr);
		...
		for ($i=$getpagebegin; $i<=$getpageend; $i++)
		{
			...
			if (!empty($links))
			{
				for ($j=0;$j<count($links);$j++)
				{
					$url=$links[$j][url];
					$pic=$links[$j][pic];
					$rowt=$dsql->GetOne("Select uid from `duomi_co_url` where tid='$id' and url='$url'");
					if(is_array($rowt)){
						$dsql->ExecuteNoneQuery("update `duomi_co_url` set succ='0',err='0' where uid=".$rowt['uid']);
						echo "{$echo_id}. $url\t<font color=red>已存在，更新采集数据</font><br>";
					}else{
						$sql="insert into `duomi_co_url`(cid,tid,url,pic,cotype) values ('$cid','$id','$url','$pic','1')";
...
function getconbyid($id,$cid='',$action2='',$index='')
{
	...
	$page = isset($page) ? intval($page) : 1;
	if($page==0) $page=1;
	$row = $dsql->GetOne("Select t.coding,t.sock,t.playfrom,t.autocls,t.classid,t.getherday,t.listconfig,t.itemconfig,c.cid,c.getconnum from `duomi_co_type` t left join `duomi_co_config` c on c.cid=t.cid where t.tid='$id'");
	$listconfig=$row['listconfig'];
	...
	//列表规则
	$listlabelRule = buildregx("{duomicms:listrule(.*?)}(.*?){/duomicms:listrule}","is");
	preg_match_all($listlabelRule,$listconfig,$listar);
	$listattrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$listar[1][0]));
	$listloopstr=$listar[2][0];
	$listattrDictionary=parduomittr($listattrStr);
	$inithit=$listattrDictionary["inithit"];
	$reverse=$listattrDictionary["reverse"];
	//页面规则
	$labelRule = buildregx("{duomicms:itemconfig(.*?)}(.*?){/duomicms:itemconfig}","is");
	...
	$csql="select count(*) as dd from `duomi_co_url` $wheresql";
	...
	$sqlStr="select * from `duomi_co_url` $wheresql ".($reverse?"order by uid desc":"")." limit 0,$getconnum";
	if($TotalResult<$getconnum)$lastpernum=$TotalResult;
	else $lastpernum=$getconnum;
	if($page>$pcount){
		if(empty($action2)){
			echo "采集内容完成\n\n<a href='admin_collect_news.php'>返回采集列表页</a>";
			echo '<embed src="skin/js/ding.wav" width="0" height="0" autostart="true" loop="true"></embed>';
			exit;
		}else{
	...
	if($TotalResult!=0){
		...
		while($rowt=$dsql->GetObject('url_list'))
		{
			...
			if($html){
				...
				if(!empty($n_title))
				{
					$rs=$dsql->GetOne("Select n_id from `duomi_co_news` where n_title like '%".$n_title."%'");
					...
					if(is_array($rs))
						$ret = update_record('duomi_co_news',"where n_id=".$rs['n_id'],$ndata);
					else
						$ret = insert_record('duomi_co_news',$ndata);
					if($ret){
						$sql = "update `duomi_co_url` set succ='1' where uid=".$rowt->uid;
						echo "{$echo_id}. {$url}\t<font color=red>保存成功</font>.<br>";
					}else{
						$sql = "update `duomi_co_url` set err=err+1 where uid=".$rowt->uid;
						echo "{$echo_id}. {$url}\t<font color=red>保存失败</font>.<br>";
					}
					$dsql->ExecuteNoneQuery($sql);
				}
	...
	$dsql->ExecuteNoneQuery("update duomi_co_type set cjtime='".time()."' where tid='$id'");
...
function getlistall($id)
{
	...
	$dsql->SetQuery("select tid from `duomi_co_type` where cid='$id' order by tid asc");
...
{
	global $dsql;
	$trow = $dsql->GetOne("select sysclsid from duomi_co_cls where clsname='$name'");
...
function updatetags($videoid, $newkeywords, $oldkeywords) {
	...
	for($i=0; $i<$arrtag_num; $i++) {
		if (!in_array($arrtag[$i], $arrold)) {
			...
			if ($arrtag[$i]) {
				$tag  = $dsql->GetOne("SELECT tagid,vids FROM duomi_tags WHERE tag='$arrtag[$i]' LIMIT 1");
				if(!$tag) {
					$dsql->ExecuteNoneQuery("INSERT INTO duomi_tags (tag,usenum,vids) VALUES ('$arrtag[$i]', '1', '$videoid')");
				} else {						
					$vids = $tag['vids'].','.$videoid;
					$dsql->ExecuteNoneQuery("UPDATE duomi_tags SET usenum=usenum+1, vids='$vids' WHERE tag='$arrtag[$i]'");
				}
			}
		}
		unset($aids);
	...
	for($i=0; $i<$arrold_num; $i++) {
		if ($arrold[$i] && !in_array($arrold[$i], $arrtag)) {
			$tag = $dsql->GetOne("SELECT vids FROM duomi_tags WHERE tag='$arrold[$i]' LIMIT 1");
			$tag['vids'] = str_replace(','.$videoid, '', $tag['vids']);
			$tag['vids'] = str_replace($videoid.',', '', $tag['vids']);
			$dsql->ExecuteNoneQuery("UPDATE duomi_tags SET usenum=usenum-1, vids='".$tag['vids']."' WHERE tag='$arrold[$i]'");
		}
	}
	$dsql->ExecuteNoneQuery("DELETE FROM duomi_tags WHERE usenum='0'");
}
...
function addtags($v_tags,$v_id)
{
	...
	if($v_tags)
	{
		...
		for($i=0; $i<$tagnum; $i++)
		{
			...
			if ($tagdb[$i]) 
			{
				$tag = $dsql->GetOne("SELECT tagid,vids FROM duomi_tags WHERE tag='$tagdb[$i]'");
				if(!$tag) {
					$dsql->ExecuteNoneQuery("INSERT INTO duomi_tags (tag,usenum,vids) VALUES ('$tagdb[$i]', '1', '$v_id')");
				}else{
					$vids = $tag['vids'].','.$v_id;
					$dsql->ExecuteNoneQuery("UPDATE duomi_tags SET usenum=usenum+1, vids='$vids' WHERE tag='$tagdb[$i]'");
				}
			}
			unset($vids);
		}
...
function import2Base($idsArray,$vtype)
{
	...
	if(count($idsArray)>0)
	{
		...
		$sql="SELECT * FROM duomi_co_news WHERE n_id IN (".$ids.")";
		$dsql->SetQuery($sql);
		...
		while($row=$dsql->GetObject('import_list'))
		{
			...
			if($tid!=''){
				...
				$v_sql="select n_id from duomi_news where 1=1 ".$v_where." order by n_id desc";
				$rs = $dsql->GetOne($v_sql);
				if(!is_array($rs)){
				$sql="INSERT INTO `duomi_news` (`n_id`, `tid`, `n_title`, `n_pic`, `n_hit`, `n_money`, `n_rank`, `n_digg`, `n_tread`, `n_commend`, `n_author`, `n_color`, `n_addtime`, `n_note`, `n_letter`, `n_isunion`, `n_recycled`, `n_entitle`, `n_outline`, `n_keyword`, `n_from`, `n_score`, `n_content`) VALUES (NULL, '".$tid."', '".addslashes($row->n_title)."', '".addslashes($row->n_pic)."', '".addslashes($row->n_hit)."', '0', '0', '0', '0', '0', '".addslashes($row->n_author)."', '', '".addslashes($row->n_addtime)."', '0', '".addslashes($row->n_letter)."', '0', '0',  '".addslashes($row->n_entitle)."', '".addslashes($row->n_outline)."', '".addslashes($row->n_keyword)."', '".addslashes($row->n_from)."', '0', '".addslashes($row->n_content)."')";
				$dsql->ExecuteNoneQuery($sql);
				}
				else
				{
				$sql="update `duomi_news`  set  n_pic='".addslashes($row->n_pic)."', n_hit='".addslashes($row->n_hit)."',n_author= '".addslashes($row->n_author)."', n_addtime='".addslashes($row->n_addtime)."', n_outline='".addslashes($row->n_outline)."', n_keyword='".addslashes($row->n_keyword)."', n_from='".addslashes($row->n_from)."', n_content='".addslashes($row->n_content)."' where n_id=".$rs['n_id'];
				$dsql->ExecuteNoneQuery($sql);
				}
				$dsql->ExecuteNoneQuery("update `duomi_co_news` set n_inbase='1',tid='$tid' where n_id=".$row->n_id);
...
function filterWord($string,$rCol)
{
	...
	if($string=='')
	return $string;
	$sql = "SELECT rColumn,uesMode,sFind,sReplace,sStart,sEnd FROM duomi_co_filters WHERE Flag=1 and cotype=1";
	$dsql->SetQuery($sql);
