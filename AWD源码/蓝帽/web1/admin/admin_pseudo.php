...
$RepWordFile=duomi_DATA."/admin/repword.txt";
...
{
	$pagesize = 30;
	$sql = " select d.v_id,d.v_name,c.body as v_content from `duomi_data` d left join `duomi_content` c on c.v_id=d.v_id";
	$dsql->SetQuery($sql);
	...
	$sql = "select d.v_id,d.v_name,c.body as v_content from `duomi_data` d left join `duomi_content` c on c.v_id=d.v_id ".$whereStr." order by v_id ASC limit $page $pagesize";
	$dsql->SetQuery($sql);
	...
	while($row = $dsql->GetArray('relacewd'))
	{
		...
		$upSql = "update `duomi_content` set body='".$row['v_content']."' where v_id =".$row[v_id];
...
{
	include(duomi_ADMIN.'/html/admin_pseudo.htm');
	exit();
}