...
if($action=="add")
{
	...
	$row = $dsql->GetOne("Select count(*) as dd from `duomi_admin` where name like '$username' ");
	if($row['dd']>0)
	{
		ShowMsg('用户名已存在！','-1');
		exit();
	...
	$inquery = "Insert Into `duomi_admin`(password,name,groupid,state) values('$pwd','$username',$groupid,1)";
...
elseif($action=="save")
{
	...
	$groupid = $groupid ? intval($groupid) : 2;
	$query = "Update `duomi_admin` set name='$username',groupid='$groupid',state='$state' $pwd where id='$id'";
...
elseif($action=="del")
{
	$rs = $dsql->ExecuteNoneQuery2("delete from `duomi_admin` where id='$id' And id<>1 And id<>'".$cuserLogin->getUserID()."'");
	if($rs>0)
	{
...
elseif($action=="delall")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from `duomi_admin` where id in ($ids) And id<>1 And id<>'".$cuserLogin->getUserID()."'");
	header("Location:admin_manager.php");
	exit();
}
else
{
	include(duomi_ADMIN.'/html/admin_manager.htm');
	exit;
}
