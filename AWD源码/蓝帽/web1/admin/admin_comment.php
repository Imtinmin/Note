...
if($action=="editcomsave")
{
	...
	$query = "update `duomi_comment` set username='$username',msg='$msg',ischeck=1 where id=$id";
...
if($action=="editgbooksave")
{
	...
	$query = "update `duomi_guestbook` set uname='$uname',msg='$msg',ischeck=1 where id=$id";
	$dsql->ExecuteNoneQuery($query);
...
{
	delcommentcache($id);
	$dsql->ExecuteNoneQuery("delete from duomi_comment where id=".$id);
...
elseif($action=="delallcomment")
{
	...
	$ids = implode(',',$e_id);
	delcommentcache($ids);
	$dsql->ExecuteNoneQuery("delete from duomi_comment where id in(".$ids.")");
	ShowMsg("成功删除所选评论！","admin_comment.php");
	exit();
}
elseif($action=="deltotalcomment")
{
	@cache_clear(duomi_ROOT.'/data/cache/review/0');
	$dsql->ExecuteNoneQuery("delete from duomi_comment");
...
elseif($action=="checkcomment")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("update duomi_comment set ischeck=1 where id in(".$ids.")");
	ShowMsg("成功审核所选评论！","admin_comment.php");
	exit();
}
elseif($action=="gbook" || $action=="viewgbook" || $action=="editgbook")
{
	include(duomi_ADMIN.'/html/admin_gbook.htm');
	exit();
}
elseif($action=="delgbook")
{
	$dsql->ExecuteNoneQuery("delete from duomi_guestbook where id=".$id);
...
elseif($action=="delallgbook")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_guestbook where id in(".$ids.")");
	ShowMsg("成功删除所选留言！","admin_comment.php?action=gbook");
	exit();
}
elseif($action=="deltotalgbook")
{
	$dsql->ExecuteNoneQuery("delete from duomi_guestbook");
...
elseif($action=="checkgbook")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("update duomi_guestbook set ischeck=1 where id in(".$ids.")");
	ShowMsg("成功审核所选留言！","admin_comment.php?action=gbook");
	exit();
}
elseif($action=="delreporterror")
{
	$dsql->ExecuteNoneQuery("delete from duomi_erradd where id=".$id);
...
elseif($action=="delallreporterror")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from duomi_erradd where id in(".$ids.")");
	ShowMsg("成功删除所选信息！","admin_comment.php?action=reporterror");
	exit();
}
else
{
	include(duomi_ADMIN.'/html/admin_comment.htm');
	exit();
}
...
{
	global $dsql;
	$dsql->setQuery("select v_id from duomi_comment where id in (".$id.")");
	$dsql->Execute("delcommentcache");
	while($row = $dsql->GetArray("delcommentcache"))
	{
		if(file_exists(duomi_DATA.'/cache/review/0/'.$row['v_id'].'.js'))
		{
			delfile(duomi_DATA.'/cache/review/0/'.$row['v_id'].'.js');
		}
	}
}
