...
include(duomi_ADMIN.'/html/admin_video.htm');
exit();
}
elseif($action=="edit")
{
	...
	//读取影片信息
	$query = "select d.*,c.body as v_content,p.body as v_playdata,p.body1 as v_downdata from duomi_data d left join `duomi_content` c on c.v_id=d.v_id left join `duomi_playdata` p on p.v_id=d.v_id  where d.v_id='$id' ";
	...
	$makePlayerSelectStr=str_replace("'","\'",makePlayerSelect(""));
	$makeDownSelectStr = str_replace("'","\'",makedownSelect(""));
	include(duomi_ADMIN.'/html/admin_video_edit.htm');
	exit();
}
elseif($action=="save")
{
	...
	{
		case "add":
			$insertSql = "insert into duomi_data(tid,v_name,v_letter,v_state,v_topic,v_hit,v_money,v_pwd,v_rank,v_actor,v_color,v_publishyear,v_publisharea,v_pic,v_addtime,v_note,v_tags,v_lang,v_score,v_scorenum,v_director,v_enname,v_commend,v_extratype,v_jq,v_nickname,v_reweek,v_douban,v_mtime,v_imdb,v_tvs,v_company,v_dayhit,v_weekhit,v_monthhit,v_len,v_total,v_daytime,v_weektime,v_monthtime,v_ver) values ('$tid','$v_name','$v_letter','$v_state','$v_topic','$v_hit','$v_money','$v_pwd','$v_rank','$v_actor','$v_color','$v_publishyear','$v_publisharea','$v_pic','$v_addtime','$v_note','$v_tags','$v_lang','$v_score','$v_scorenum','$v_director','$v_enname','$v_commend','$v_extratype','$v_jq','$v_nickname','$v_reweek','$v_douban','$v_mtime','$v_imdb','$v_tvs','$v_company','$v_dayhit','$v_weekhit','$v_monthhit','$v_len','$v_total','$v_daytime','$v_weektime','$v_monthtime','$v_ver')";
			if($dsql->ExecuteNoneQuery($insertSql))
			{
				$v_id = $dsql->GetLastID();
				$dsql->ExecuteNoneQuery("INSERT INTO `duomi_content`(`v_id`,`tid`,`body`) VALUES ('$v_id','$tid','$v_content')");
				$dsql->ExecuteNoneQuery("INSERT INTO `duomi_playdata`(`v_id`,`tid`,`body`,`body1`) VALUES ('$v_id','$tid','$v_playdata','$v_downdata')");
				addtags($v_tags,$v_id);
				clearTypeCache();
				//推送新增
				if($ping==1)	
			...
			{
				$gerr = $dsql->GetError();
				ShowMsg("把数据保存到数据库主表 `duomi_data` 时出错。".str_replace('"','',$gerr),"javascript:;");
				exit();
			}
		break;
		case "edit":
			...
			$updateSql = "tid = '$tid',v_name = '$v_name',v_letter = '$v_letter',v_state = '$v_state',v_topic = '$v_topic',v_hit = '$v_hit',v_money = '$v_money',v_pwd = '$v_pwd',v_rank = '$v_rank',v_actor = '$v_actor',v_color = '$v_color',v_publishyear = '$v_publishyear',v_publisharea = '$v_publisharea',v_pic = '$v_pic',v_note = '$v_note',v_tags = '$v_tags',v_lang='$v_lang',v_director='$v_director',v_enname='$v_enname',v_extratype='$v_extratype',v_jq='$v_jq',v_nickname='$v_nickname',v_reweek='$v_reweek',v_douban='$v_douban',v_mtime='$v_mtime',v_imdb='$v_imdb',v_tvs='$v_tvs',v_company='$v_company',v_dayhit='$v_dayhit',v_weekhit='$v_weekhit',v_monthhit='$v_monthhit',v_len='$v_len',v_total='$v_total',v_score='$v_score',v_scorenum='$v_scorenum',v_ver='$v_ver'";
			if(!empty($isupdatetime)) $updateSql .= ",v_addtime='$v_addtime'";
			$updateSql = "update duomi_data set ".$updateSql." where v_id=".$v_id;
			if(!$dsql->ExecuteNoneQuery($updateSql))
			{
				ShowMsg('更新影片出错，请检查',-1);
				exit();
			}
			if(!$dsql->ExecuteNoneQuery("update `duomi_content` set `body`='$v_content' where v_id='$v_id'"))
			{
				ShowMsg("更新影片内容时出错，请检查原因！",-1);
				exit();
			}
			if(!$dsql->ExecuteNoneQuery("update `duomi_playdata` set `body`='$v_playdata',`body1`='$v_downdata' where v_id='$v_id'"))
			{
			...
			if($cfg_runmode=='0'){
				$trow = $dsql->GetOne("select ishidden from duomi_type where tid=".$tid);
				if($trow['ishidden']==1){
...
elseif($action=="lock")
{
	$back=$Pirurl;
	$id = isset($id) && is_numeric($id) ? $id : 0;
	$dsql->ExecuteNoneQuery("update `duomi_data` set v_isunion = '1' where v_id='$id'");
	ShowMsg("影片锁定成功",$back);
	exit();
}
elseif($action=="unlock")
{
	$back=$Pirurl;
	$id = isset($id) && is_numeric($id) ? $id : 0;
	$dsql->ExecuteNoneQuery("update `duomi_data` set v_isunion = '0' where v_id='$id'");
	ShowMsg("影片解锁成功",$back);
	exit();
}
elseif($action=="lockall")
{
	...
	$dsql->SetQuery("select v_id,v_isunion from `duomi_data` where v_id in (".implode(',',$e_id).")");
	$dsql->Execute("lockdata");
	while($rs = $dsql->GetArray("lockdata"))
	{
		$dsql->ExecuteNoneQuery("update `duomi_data` set v_isunion = '".($rs['v_isunion']==1?0:1)."' where v_id=".$rs['v_id']);
	}
...
elseif($action=="del")
{
	$back=$Pirurl;
	$id = isset($id) && is_numeric($id) ? $id : 0;
	$vtypeAndPic=$dsql->GetOne("select tid,v_pic,v_addtime,v_enname from duomi_data where v_id=".$id);
	...
	$dsql->ExecuteNoneQuery("delete from duomi_data where v_id=".$id);
	$dsql->ExecuteNoneQuery("delete From `duomi_content` where v_id='$id'");
	$dsql->ExecuteNoneQuery("delete From `duomi_playdata` where v_id='$id'");
	clearTypeCache();
	ShowMsg("影片删除成功",$back);
	exit();
}
elseif($action=="restoreall")
{
	...
	$ids = implode(',',$e_id);
	$sqlStr="update duomi_data set v_recycled=0 where v_id in(".$ids.")";
...
elseif($action=="delall")
{
	...
	$ids = implode(',',$e_id);
	/*$sqlStr="select v_id,tid,v_enname,v_addtime from duomi_data where v_id in(".$ids.")";
	...
	$dsql->ExecuteNoneQuery("delete from duomi_data where v_id in(".$ids.")");
	$dsql->ExecuteNoneQuery("delete From `duomi_content` where v_id in(".$ids.")");
	$dsql->ExecuteNoneQuery("delete From `duomi_playdata` where v_id in(".$ids.")");
	clearTypeCache();
	ShowMsg("影片删除成功",$back);
	exit();
}
elseif($action=="psettopic")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("update duomi_data set v_topic=".$ptopic." where v_id in(".$ids.")");
	ShowMsg("专题操作成功",$back);
	exit();
}
elseif($action=="psettype")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("update duomi_data set tid=".$movetype." where v_id in(".$ids.")");
...
elseif($action=="deltypedata")
{
	...
	$dsql->ExecuteNoneQuery("delete from duomi_data where tid=".$movetype);
...
elseif($action=="hide")
{
	...
	$dsql->ExecuteNoneQuery("update duomi_data set v_recycled=1 where v_id=".$id);
	ShowMsg("隐藏影片成功",$back);
	exit();
...
elseif($action=="restore")
{
	...
	$dsql->ExecuteNoneQuery("update duomi_data set v_recycled=0 where v_id=".$id);
	ShowMsg("还原影片成功",$back);
	exit();	
}
else
{
	...
	include(duomi_ADMIN.'/html/admin_video_main.htm');
	exit();
}
...
function makeRankSelect($selectName,$strSelect,$rankId)
{
	...
	$sql="select rank,membername from duomi_arcrank order by id asc";
	if($cfg_iscache){
...
function clearTypeCache()
{
	...
	if($cfg_iscache)
	{
		$TypeCacheFile=duomi_DATA."/cache/".$cfg_cachemark.md5('array_Type_Lists_all').".inc";
...
function updatetags($videoid, $newkeywords, $oldkeywords) {
	...
	for($i=0; $i<$arrtag_num; $i++) {
		if (!in_array($arrtag[$i], $arrold)) {
			...
			if ($arrtag[$i]) {
				$tag  = $dsql->GetOne("SELECT tagid,vids FROM duomi_tags WHERE tag='$arrtag[$i]'");
				if(!$tag) {
	...
	for($i=0; $i<$arrold_num; $i++) {
		if ($arrold[$i] && !in_array($arrold[$i], $arrtag)) {
			$tag = $dsql->GetOne("SELECT vids FROM duomi_tags WHERE tag='$arrold[$i]'");
...
function zzget($txt)
{
	if($txt=='area'){$txt=duomi_DATA."/admin/publisharea.txt";}
	elseif($txt=='year'){$txt=duomi_DATA."/admin/publishyear.txt";}
	elseif($txt=='yuyan'){$txt=duomi_DATA."/admin/publishyuyan.txt";}
	elseif($txt=='ver'){$txt=duomi_DATA."/admin/verlist.txt";}
