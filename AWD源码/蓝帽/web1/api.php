...
/******************************
require_once("duomiphp/main.class.php");
******************************/
...
function cj()
{
	...
	$sql = "select d.*,p.body as v_playdata,p.body1 as v_playdata1,t.tname from duomi_data d left join `duomi_type` t on t.tid=d.tid left join `duomi_playdata` p on p.v_id=d.v_id where d.v_recycled=0 ";
	$sql1 = "select count(*) as dd from duomi_data where v_recycled=0 ";
	...
	while($row=$dsql->GetObject('video_c'))
		{
			...
			$query = "select body  from duomi_content  where v_id='$row->v_id' ";
...
function vlist()
{
	...
	$sql ="select d.v_id,d.v_name,d.v_state,d.v_note,d.tid,d.v_addtime,p.body as v_playdata,p.body1 as v_playdata1,t.tname from duomi_data d left join `duomi_type` t on t.tid=d.tid left join `duomi_playdata` p on p.v_id=d.v_id where d.v_recycled=0 ";
	$sql1 = "select count(*) as dd from duomi_data where v_recycled=0 ";
	...
	//分类列表开始
	$xml .= "<class>";
	$sqltype = "select * from duomi_type where tptype=0 ";
