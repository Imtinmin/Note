...
******************************/
require_once('../duomiphp/link.func.php');
require_once('../data/admin/weixin.php');
require_once('../data/common.inc.php');

if(isopen=="n"){die('<br>多米影视微信公众号已<font color="#ff0000">关闭</font>!<br><br>请联系管理员<font color="#ff00ff">开启</font>！');}
define("TOKEN", "weixin");	 
					...
					case 'text':
						...
						{	
							...
							$newsTplFooter="</Articles>
									...
									$sql = "SELECT * FROM `duomi_data` WHERE `v_name` like '%".$keyword."%'  LIMIT 0 ,".sql_num."";
						...
						elseif(is_numeric($keyword))
						{
							...
							$row=$this->dsql->GetOne("SELECT * FROM `duomi_data` WHERE `v_id` =$keyword");
							$psd=$row['v_psd'];
							$vname=$row['v_name'];
