...
class Config extends BaseService implements IConfig
{
     ...
     */
    function getCustomTeplateInfo($condition)
    {
    ...
    {
        $custom_template_model = new CustomTemplateModel();
        $custom_template_model_info = $custom_template_model->getInfo(["SHOP_ID"=>$shop_id]);
        $data['shop_id'] = $shop_id;
        $data['template_name'] = $template_name;
        $data['template_data'] = $template_data;
        $data['modify_time'] = time();
        if(empty($custom_template_model_info)){
            $data['create_time'] = time();
            $res = $custom_template_model->save($data); 
        }else{
            $res = $custom_template_model->save($data, [
                'id' => $custom_template_model_info["id"]
            ]);
        }
        return $res;
    }   
