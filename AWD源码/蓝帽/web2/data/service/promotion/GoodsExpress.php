...
class GoodsExpress extends BaseService{
    ...
    */
    ...
    public function getExpressCompany($shop_id, $goods_sku_list, $province_id, $city_id)
    {
        ...
        $list = $express_company_model->getQuery(['shop_id' => $shop_id, 'is_enabled' => 1], 'co_id,company_name,is_default', 'orders');
        if(!empty($list))
        {
            $new_list = array();
            foreach ($list as $k => $v)
    ...
    {
        $express_company_model = new NsOrderExpressCompanyModel();
        $list = $express_company_model->getQuery(['shop_id' => $shop_id], '*', '');
        return $list;
    }
    /**
     * 获取店铺物流公司
