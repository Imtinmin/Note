...
//             var_dump($goods_info["sku_picture_list"]);
    ...
    public function goodsList()
    {
        ...
        $spec = isset($_GET["spec"]) ? $_GET["spec"] : ""; // 规格值
        ...
        if ($category_id != "") {
            ...
            if($goods_attribute["spec_id_array"] != ""){
                $goods_spec_array = $this->goods->getGoodsSpecQuery(["spec_id"=>["in",$goods_attribute["spec_id_array"]]]);              
...
//         $spec_index_num = strpos($url_parameter, "&spec");
    ...
    public function test(){
        $goods = new GoodsService();
        $spec_array = array("12:28","12:25","2:7");
        $spec_count = count($spec_array);
        $spec_where = "";
        if($spec_count > 0){
            foreach ($spec_array as $k => $v) {
