...
function echoChannel($typeId)
{
	...
	if (strpos(" ,".getHideTypeIDS().",",",".$typeId.",")>0) {showMsg("该分类无影片或隐藏","-1");
		exit();};
	if ($cfg_user == 1){
        if (!getUserAuth($typeId, "list")){showMsg("您没有权限浏观该分类，请升级用户组!","-1");
		exit();}
    }
	...
	if($typeId!="")
		...
		$extrasql = "";
	$sql="select count(*) as dd from duomi_data where (tid in (".$typeIds.") ".$extrasql.")";
	$row = $dsql->GetOne($sql);
	if(is_array($row))
	{
		$TotalResult = $row['dd'];
