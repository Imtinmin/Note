...
function makeAllmovie($by='video')
{
	...
	switch ($by){
		case 'video':
			$templatePath="/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/map.html";
		break;
		case 'news':
			$templatePath="/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/newsmap.html";
		break;
	}
	$content=loadFile(duomi_ROOT.$templatePath);
	$content=$mainClassObj->parseTopAndFoot($content);
	...
	switch ($by){
		case 'video':
			$allmoviename=duomi_ROOT."/allmovie".getfileSuffix();
		...
		break;
		case 'news':
			$allmoviename=duomi_ROOT."/allnews".getnewsfileSuffix();
			createTextFile($content,$allmoviename);
...
function makeVideoJs($by='video')
{
	...
	switch ($by){
		case 'video':
			$templatePath="/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/js.html";
		break;
		case 'news':
			$templatePath="/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/newsjs.html";
		break;
	}
	$content=loadFile(duomi_ROOT.$templatePath);
	$content=$mainClassObj->parseTopAndFoot($content);
...
function makeDay()
{
	...
	$pagesize=30;
	if(!$pCount){
	$rowc=$dsql->GetOne("SELECT count(*) as dd FROM `duomi_data` WHERE `v_wrong`=0 and v_recycled=0 ".$wheresql);
	$totalnum = $rowc['dd'];
	...
	$sql="select v_id from duomi_data where v_wrong=0 and v_recycled=0 $wheresql limit $limitstart,$pagesize";
	...
	{
		$ids="";
		$sqlt="SELECT tid from duomi_data where v_wrong=0 and v_recycled=0 ".$wheresql." GROUP BY tid";
		$dsql->SetQuery($sqlt);
...
function makeNewsDay()
{
	...
	$pagesize=100;
	if(!$pCount){
	$rowc=$dsql->GetOne("SELECT count(*) as dd FROM `duomi_news` WHERE n_recycled=0 ".$wheresql);
	$totalnum = $rowc['dd'];
	...
	$limitstart = ($currentPage-1) * $pagesize;
	if($limitstart<0) $limitstart=0;
	$sql="select n_id from duomi_news where n_recycled=0 $wheresql limit $limitstart,$pagesize";
	...
	{
		$ids="";
		$sqlt="SELECT tid from duomi_news where n_recycled=0 ".$wheresql." GROUP BY tid";
		$dsql->SetQuery($sqlt);
		$dsql->Execute('makeDayt');
		while($rowt=$dsql->GetObject('makeDayt'))
		{
...
function makeArticleById($vId)
{
	...
	$playn = 0;
	$row=$dsql->GetOne("Select * From `duomi_news` where n_id='$vId'");
	if(!is_array($row)){
	...
	$contentTemplatePath = "/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/".$contentTmpName;
	...
	if (strpos($content,"{news:subcontent}")>0||strpos($content,"{news:subtitle}")>0||strpos($content,"{news:subpagenumber}")>0){	
		...
		for($i=0;$i<count($desarr);$i++)
		{
			...
			$substrarr=$mainClassObj->parduomittr($substr);
			if(count($desarr)>1)
			...
			createTextFile($tmp,duomi_ROOT.$contentLink,"");
			echoEach($row["n_title"],$i,'..'.$contentLink,"article");
		}
	}else{
		createTextFile($content,duomi_ROOT.$contentLink,"");
		echoEach($row["n_title"],$i,'..'.$contentLink,"article");
	}
}
...
function makeContentById($vId)
{
	...
	$playTemplatePath = "/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/".$playTemFileName;
	...
	$dsql->ExecuteNoneQuery("update duomi_data set v_ismake='1' where v_id=".$vId);
	return $stringecho;
}
	...
	if($GLOBALS['cfg_ismakeplay']==1){
		for($i=0;$i<$playArr[0];$i++)
		{
			...
			foreach ($playArr[2][$i] as $n=>$play){
				...
				createTextFile($tmp1,duomi_ROOT.$playLink,"");
		...
		createTextFile($content,duomi_ROOT.$playLink,"");
	}
...
function makeContentByChannel($channel,$isIncludeSub,$makeNoncreate=0)
{
	...
	$currentPage = empty($page) ? 1 : intval($page);
	$rowc = $dsql->GetOne("Select count(*) as dd From `duomi_data` $sqlStr");
	$totalnum = $rowc['dd'];
	if (isTypeHide($channel) || $totalnum==0){
		if (empty($action2)){
			echo "该分类<font color='red'>".$typeZnName."</font>无视频或被隐藏<br>";
			return true;
	...
	$limitstart = ($currentPage-1) * $pagesize;
	if($limitstart<0) $limitstart=0;
	$sql="select v_id from duomi_data $sqlStr limit ".($makeNoncreate?0:$limitstart).",$pagesize";
...
function makeArticleByChannel($channel,$isIncludeSub)
{
	...
	$currentPage = empty($page) ? 1 : intval($page);
	$rowc = $dsql->GetOne("Select count(*) as dd From `duomi_news` $sqlStr");
	$totalnum = $rowc['dd'];
	...
	$limitstart = ($currentPage-1) * $pagesize;
	if($limitstart<0) $limitstart=0;
	$sql="select n_id from duomi_news $sqlStr limit $limitstart,$pagesize";
	echoBegin ("正在开始生成栏目<font color='red'>".$typeZnName."</font>的内容页,当前是第<font color='red'>".$currentPage."</font>页,共<font color='red'>".$TotalPage."</font>页<br>","content");
	$dsql->SetQuery($sql);
	$dsql->Execute('makeContentByChannel');
	while($row=$dsql->GetObject('makeContentByChannel'))
...
function makeChannelById($typeId)
{
	...
	$sql="select count(*) as dd from duomi_data where tid in (".$typeIds.")";
	$row = $dsql->GetOne($sql);
	if(is_array($row))
	{
		$TotalResult = $row['dd'];
	...
	if($TotalResult == 0||strpos($content,'{/duomicms:channellist}')===false){
		...
		$content=$mainClassObj->parseIf($content);
		createTextFile($content,duomi_ROOT.str_replace($GLOBALS['cfg_cmspath'],"",getChannelPagesLink($currentTypeId,1,'all')));
	...
	for($i=$mstart;$i<=$mend;$i++){
		...
		createTextFile($tmp,duomi_ROOT.str_replace($GLOBALS['cfg_cmspath'],"",getChannelPagesLink($currentTypeId,$i,'all')));
...
function makeNewsChannelById($typeId)
{
	...
	if($TotalResult == 0){
		...
		$content=$mainClassObj->parseIf($content);
		createTextFile($content,duomi_ROOT.$channelLink);
...
function makeTopicIndex()
{
	...
	$row = $dsql->GetOne("select template from duomi_topic");
	$templatePath="/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/topicindex.html";
	$rowc=$dsql->GetOne("select count(*) as dd from duomi_topic");
	...
	for($i=1;$i<=$pCount;$i++)
	{
		...
		$content=$mainClassObj->parseIf($content);
		$topicindexname=duomi_ROOT."/".$GLOBALS['cfg_album_name']."/index".($i==1?'':$i).$GLOBALS['cfg_filesuffix2'];
...
{
	global $dsql;
	$dsql->SetQuery("select id from duomi_topic order by sort asc");
...
{
	global $dsql,$cfg_iscache,$mainClassObj;
	$sql="select * from duomi_topic where id =".$topicId;
	$row = $dsql->GetOne($sql);
	if(!is_array($row)) exit("不存在此专题");
						$sql="select vod from duomi_topic where id='$topicId'";
						$rows=array();
						$dsql->SetQuery($sql);
						$dsql->Execute('al');
	...
	if($cfg_iscache){
		if(chkFileCache($cacheName)){
			...
			$content = str_replace("{duomicms:topicname}",$topicName,$content);
			$content = str_replace("{duomicms:topicdes}",$topicDes,$content);
			$content = str_replace("{duomicms:topickeyword}",$topicKeyword,$content);
			$content = str_replace("{duomicms:currrent_topic_id}",$currrent_topic_id,$content);
			$content = str_replace("{duomicms:topicpic}",$topicPic,$content);
			setFileCache($cacheName,$content);
		}
	}else{
			$content = parseCachePart("topic",$topicTemplatePath);
	...
	if($TotalResult == 0){
		...
		$content=$mainClassObj->parseIf($content);
		$topiclink=duomi_ROOT.str_replace($GLOBALS['cfg_cmspath'],"",getTopicLink($topicEnname,1));
		...
		for($i=1;$i<=$pCount;$i++){
			...
			$content=$mainClassObj->parseIf($content);
			$topiclink=duomi_ROOT.str_replace($GLOBALS['cfg_cmspath'],"",getTopicLink($topicEnname,$i));
			createTextFile($content,$topiclink);
...
function makeCustomInfo($templatename)
{
	...
	$self_str="self_";
	$pcount=0;
	$templatePath = "/duomiui/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/".$templatename; 
	...
	if(strpos($content, "{/duomicms:customvideolist}")>0){
		$sql="select count(*) as dd from duomi_data where v_recycled=0".$whereStr;
		$row = $dsql->GetOne($sql);
		if(is_array($row))
		{
			$TotalResult = $row['dd'];
	...
	for($i=1;$i<=100;$i++)
	{
		...
		createTextFile ($tmp,duomi_ROOT.$dir);
...
function makeBaidu()
{
	...
	if ($flag!=1){
		...
		for($i=1;$i<=$pCount;$i++){
			...
			$sql="select d.v_id,d.v_addtime,d.tid,d.v_enname from duomi_data d order by d.v_addtime desc limit $limitstart,$pagesize";
			$dsql->SetQuery($sql);
			...
			createTextFile($baiduStr,duomi_ROOT."/xml/baidu".$xmlUrl.".xml");
...
function makeGoogle()
{
	...
	if ($flag!=1){
		...
		for($i=1;$i<=$pCount;$i++){
			$limitstart = ($i-1) * $pagesize;
			$sql="select v_id,v_addtime,v_enname,tid from duomi_data order by v_addtime desc limit $limitstart,$pagesize";
			$dsql->SetQuery($sql);
			...
			$googleStr=$googleStr;
			if ($i==1) $xmlUrl=""; else $xmlUrl="_".$i;
			createTextFile($googleStr,duomi_ROOT."/xml/google".$xmlUrl.".xml");
...
function makeRss()
{
	...
	if ($flag!=1){
		...
		$makenum = empty($makenum) ? 100 : intval($makenum);
		$sql="select d.v_id,d.v_name,d.v_pic,d.v_actor,d.v_addtime,d.v_enname,d.tid,c.body as v_des from duomi_data d left join duomi_content c on c.v_id=d.v_id order by d.v_addtime desc limit 0,$makenum";
		$dsql->SetQuery($sql);
		...
		$rssStr =  "<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n
					...
					<image>\n
						<url>".$GLOBALS['cfg_basehost']."/images/logo.gif</url>\n
					</image>\n";
		...
		$rssStr .= "</channel></rss>\n";
		createTextFile($rssStr,duomi_ROOT."/xml/rss.xml");
...
function makeBaidux()
{
	global $dsql,$flag,$makenum,$allmakenum;
	if ($flag!=1){
		return "<br><div align=center><b>生成百度结构化数据</b>： 总输出数量<input type='text' id='allmakenum' value='10000'>每页数量<input type='text' id='makenum' value='2000'> <input type='button' class='rb1' value='开始生成' onclick=\"javascript:location.href='?action=baidux&flag=1&allmakenum='+$('allmakenum').value+'&makenum='+$('makenum').value\" /></div>";
	}else{
		$sqlt="select tid,tname from duomi_type";
		$dsql->SetQuery($sqlt);
		$dsql->Execute('ztype');
		...
		for($i=1;$i<=$pCount;$i++){
			$limitstart = ($i-1) * $pagesize;
			$sql="select a.*,b.* from duomi_data as a,duomi_content as b WHERE a.v_id=b.v_id order by a.v_id desc limit $limitstart,$pagesize";
			$dsql->SetQuery($sql);
...
$baiduxStr .= "</urlset>\n";
			$baiduxStr=$baiduxStr;
			if ($i==1) $xmlUrl=""; else $xmlUrl="_".$i;
			createTextFile($baiduxStr,duomi_ROOT."/xml/baidux".$xmlUrl.".xml");
