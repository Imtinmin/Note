...
function getTestItemRule($tid,$previewurl,$previewpic='')
{
	...
	global $dsql;
	$row = $dsql->GetOne("Select coding,sock,playfrom,autocls,classid,itemconfig,listconfig,getherday from `duomi_co_type` where tid='$tid'");
	$itemconfig=$row['itemconfig'];
	$listconfig=$row['listconfig'];
	$labelRule1 = buildregx("{duomicms:listrule(.*?)}(.*?){/duomicms:listrule}","is");
	preg_match_all($labelRule1,$listconfig,$ar1);
	$listattrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar1[1][0]));
	$listDictionary=parduomittr($listattrStr);
...
function getNewsTestItemRule($tid,$previewurl,$previewpic='')
{
	...
	if($tid==0) return false;
	global $dsql;
	$row = $dsql->GetOne("Select coding,sock,playfrom,autocls,classid,itemconfig,getherday from `duomi_co_type` where tid='$tid'");
	$itemconfig=$row['itemconfig'];
	$coding=$row['coding'];
	$sock=$row['sock'];
	$playfrom=$row['playfrom'];
...
{
	if($trimRule=='' || strpos($trimRule,'{duomicms:trim')=== false){
		return $trimvalue;
	}else{
		$labelRule = buildregx("{duomicms:trim(.*?)}(.*?){/duomicms:trim}","is");
...
function getrulevalue($content,$str)
{
	if(!empty($content) && !empty($str)){
		$labelRule = buildregx("{duomicms:".$str."}(.*?){/duomicms:".$str."}","is");
