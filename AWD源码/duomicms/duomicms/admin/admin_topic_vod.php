...
if(empty($action))
{
	...
	$sql="select * from duomi_topic where id=$tid"; //获取之前的影片id'数据
	$dsql->SetQuery($sql);
	$dsql->Execute('al') ;
...
if($action=="topicadd")
{
	...
	$zinfo="$info1$vid";
	$usql = "update duomi_topic set vod='$zinfo' where id='$tid'";
...
elseif($action=="topicdel")
{
	...
	$sql = "update duomi_topic set vod='$del' where id='$tid'";
...
{	
	...
	$all="$info1$allstr";
	$usql = "update duomi_topic set vod='$all' where id='$tid'";
	if(!$dsql->ExecuteNoneQuery($usql))
	{echo "'alert('插入数据失败')";}
	header("Location: admin_topic_vod.php?tid=$tid"); 
	exit;	
...
{
	$dallsql = "update duomi_topic set vod='0' where id='$tid'";
...
include(duomi_ADMIN.'/html/admin_topic_vod.htm');
exit();
}
elseif($action=="save")
{
	...
	{
		case "add":
			$insertSql = "insert into duomi_data(tid,v_name,v_letter,v_state,v_topic,v_hit,v_money,v_rank,v_actor,v_color,v_publishyear,v_publisharea,v_pic,v_addtime,v_note,v_tags,v_lang,v_director,v_enname,v_commend,v_extratype) values ('$tid','$v_name','$v_letter','$v_state','$v_topic','$v_hit','$v_money','$v_rank','$v_actor','$v_color','$v_publishyear','$v_publisharea','$v_pic','$v_addtime','$v_note','$v_tags','$v_lang','$v_director','$v_enname','$v_commend','$v_extratype')";
...
function isVideoMake($v_id,$contentUrl)
{
	...
	if(file_exists('..'.$contentUrl)){
		echo "<img src='skin/images/yes.gif' border='0' title='点击生成HTML' />";
	}else{
		echo "<img src='skin/images/no.gif' border='0' title='点击生成HTML' />";
	}
	echo "</a>";
}
