...
if($action=='')
{
  ...
  <tr>
    <td align="center"><span id="xmllist"><h1><br><br>执行完毕:';
...
elseif($action=="bindsubmit")
{
	if(is_numeric($tid))
	{
		if(!empty($v_oldtype))
		{
			$row = $dsql->GetOne("select tid,unionid from duomi_type where tid='$v_oldtype'");
			if(is_array($row)){
				$unionid=delBindId($curid,$row["unionid"]);
				$dsql->ExecuteNoneQuery("update duomi_type set unionid='$unionid' where tid=".$row['tid']);
			}
		}
		$row = $dsql->GetOne("select tid,unionid from duomi_type where tid='$tid'");
		if(is_array($row)){
		...
		$row = $dsql->GetOne("select tid,unionid from duomi_type where concat(',',unionid,',') like '%,".$curid.",%'");
		if(is_array($row)){
			$unionid=delBindId($curid,$row["unionid"]);
			$dsql->ExecuteNoneQuery("update duomi_type set unionid='$unionid' where tid=".$row['tid']);
		}
...
<title>资源库采集程序</title>
<link rel="stylesheet" type="text/css" href="skin/images/res.css"/>
