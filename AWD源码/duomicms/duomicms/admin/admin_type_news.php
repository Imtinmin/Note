...
if($action=="add")
{
	...
	$in_query = "insert into `duomi_type`(upid,torder,tname,tenname,templist,templist_1,keyword,description,tptype) Values('$upid','$torder','$tname','$tenname','$templist','$templist_1','$keyword','$description',1)";
...
elseif($action=="edit")
{
	...
	if (!is_numeric($torder)) $torder=1;
	$dsql->ExecuteNoneQuery("update duomi_type set tname='$tname',tenname='$tenname',templist='$templist',templist_1='$templist_1',keyword='$keyword',description='$description',torder='$torder' where tid=".$id);
	clearTypeCache();
	}
	header("Location:admin_type_news.php");
	exit;
...
{
include(duomi_ADMIN.'/html/admin_type_news.htm');
exit();
}
...
function clearTypeCache()
{
	...
	if($cfg_iscache)
	{
		$TypeCacheFile=duomi_DATA."/cache/".$cfg_cachemark."obj_get_type_list_1.inc";
