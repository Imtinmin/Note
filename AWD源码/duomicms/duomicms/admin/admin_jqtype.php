...
if($action=="add")
{
	...
	$in_query = "insert into `duomi_jqtype`(tid,tname,ishidden) Values('','$tname',0)";
...
elseif($action=="hide")
{
	...
	$dsql->ExecuteNoneQuery("update duomi_jqtype set ishidden=1 where tid=".$id);
	clearTypeCache();
...
{
	$dsql->ExecuteNoneQuery("update duomi_jqtype set ishidden=0 where tid=".$id);
	clearTypeCache();
	header("Location:admin_jqtype.php");
	exit;
}
...
elseif($action=="edit")
{
	...
	$dsql->ExecuteNoneQuery("update duomi_jqtype set tname='$tname' where tid=".$id);
	clearTypeCache();
	}
	header("Location:admin_jqtype.php");
	exit;
...
{
include(duomi_ADMIN.'/html/admin_jqtype.htm');
exit();
}
...
function clearTypeCache()
{
	...
	if($cfg_iscache)
	{
		$TypeCacheFile=duomi_DATA."/cache/".$cfg_cachemark."obj_get_jqtype_list_0.inc";
...
function delVideoTypeById($id)
{
	...
	if (isExistSub($id,"type"))
	{
		...
		$dsql->ExecuteNoneQuery("delete from duomi_jqtype where tid=".$id);
		clearTypeCache();
...
function isExistSub($bigType,$operFlag)
{
	...
	{
		case "type":
			$row = $dsql->GetOne("select count(*) as dd from duomi_jqtype where upid=".$bigType);
		break;
		case "video":
...
{
	global $dsql;
	$sql="select count(*) as dd from duomi_data where v_jq like '%$tname%'";
	$row = $dsql->GetOne($sql);
	return $row[dd];
}
