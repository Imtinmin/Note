...
if($action=="downpic")
{
	include_once(duomi_DATA."/config.ftp.php");
	@session_write_close();
	$isDownOk=false;
	$wheresql="where instr(v_pic,'#err')=0".($app_ftp==0?"":" and instr(v_pic,'$app_ftpurl')=0")." and instr(v_pic,'http')<>0";
	$trow = $dsql->GetOne("Select count(*) as dd From `duomi_data` $wheresql");
	$totalnum = $trow['dd'];
	...
	$dsql->SetQuery("Select v_id,v_name,v_pic From `duomi_data` $wheresql order by v_addtime desc limit 0,$pagesize");
	$dsql->Execute('getpic');
	while($row=$dsql->GetObject('getpic'))
	{
		...
		$query = "Update `duomi_data` set v_pic='$picUrl' where v_id='$v_id'";
...
elseif($action=="watermark")
{
	...
	$dsql->SetQuery("Select v_id,v_pic,v_name From `duomi_data` $wheresql order by v_addtime desc limit 0,$pagesize");
	$dsql->Execute('getpic');
	while($row=$dsql->GetObject('getpic'))
	{
		...
		if($errno===true)
		{
			...
			$dsql->ExecNoneQuery("update duomi_data set v_pic= '".$row->v_pic."#marked' where v_id = ".$row->v_id);
		}else 
		{
			echo "数据<font color=red>".$row->v_name."</font>的图片水印失败,错误号$errno";
			$dsql->ExecNoneQuery("update duomi_data set v_pic= '".$row->v_pic."#error_".$errno."_marked' where v_id = ".$row->v_id);
		}
	}
...
elseif($action=="ftppic")
{
	...
	include(duomi_ADMIN.'/html/admin_datarelate.htm');
	exit();
}
elseif($action=="saveftppic")
{
	$configfile = duomi_DATA."/config.ftp.php";
...
elseif($action=="batchsubmit")
{
	...
	if($v_field=="v_content"){
		$sql="select v_id,body from `duomi_content` where body like '%".$v_str1."%'";
		$dsql->SetQuery($sql);
		$dsql->Execute('replacestr');
		while($row=$dsql->GetObject('replacestr'))
		{
			if($dsql->ExecuteNoneQuery("update `duomi_content` set body='".addslashes(str_replace($v_str1,$v_str2,$row->body))."' where v_id='".$row->v_id."'"))
			{echo  "<tr><td>ID为".$row->v_id."的数据替换成功</td></tr>";}
		}
	}elseif($v_field=="v_playdata"){
		$sql="select v_id,body from `duomi_playdata` where body like '%".$v_str1."%'";
		$dsql->SetQuery($sql);
		$dsql->Execute('replacestr');
		while($row=$dsql->GetObject('replacestr'))
		{
			$dsql->ExecuteNoneQuery("update `duomi_playdata` set body='".addslashes(str_replace($v_str1,$v_str2,$row->body))."' where v_id='".$row->v_id."'");
			echo  "<tr><td>ID为".$row->v_id."的数据替换成功</td></tr>";
		}
	}elseif($v_field=="v_downdata"){
		$sql="select v_id,body1 from `duomi_playdata` where body1 like '%".$v_str1."%'";
		$dsql->SetQuery($sql);
		$dsql->Execute('replacestr');
		while($row=$dsql->GetObject('replacestr'))
		{
			$dsql->ExecuteNoneQuery("update `duomi_playdata` set body1='".addslashes(str_replace($v_str1,$v_str2,$row->body1))."' where v_id='".$row->v_id."'");
			echo  "<tr><td>ID为".$row->v_id."的数据替换成功</td></tr>";
		}
	}else{
		$sql="select v_id,".$v_field." from `duomi_data` where ".$v_field." like '%".$v_str1."%'";
		$dsql->SetQuery($sql);
		$dsql->Execute('replacestr');
		while($row=$dsql->GetObject('replacestr'))
		{
			$query="update `duomi_data` set ".$v_field."='".addslashes(str_replace($v_str1,$v_str2,$row->$v_field))."' where v_id='".$row->v_id."'";
			$dsql->ExecuteNoneQuery($query);
			echo  "<tr><td>ID为".$row->v_id."的数据替换成功</td></tr>";
		}
	}
...
{
	include(duomi_DATA."/config.ftp.php");
	$sql = "select v_id,v_pic from duomi_data order by v_id ASC "; 
	$dsql->SetQuery($sql);
	...
	while($row = $dsql->GetArray('existpic'))
	{
		if($row[v_pic]!=''&&strpos($row[v_pic],'uploads/')===0){
			...
			{
				$upSql = "update `duomi_data` set v_pic='' where v_id =".$row[v_id];
		...
		foreach ($folder as $file)
		{
			if($file['filetype']=='folder')
			{
				...
				foreach ($folder2 as $imfile)
				{
					...
					$sql = "select v_id from duomi_data where v_pic ='". $imdir."' or v_pic like '". $imdir."%marked'";
					$dsql->SetQuery($sql);
					$dsql->Execute('querypic');
					if($dsql->GetTotalRow('querypic')==0)
					{	
						unlink('../'.$imdir);
...
elseif($action=="sql")
{
	include(duomi_ADMIN.'/html/admin_datarelate_sql.htm');
	exit();
}
elseif($action=="result"){
	include(duomi_ADMIN.'/html/admin_datarelate_result.htm');
	exit();
}
elseif($action=="randomset")
{
	include(duomi_ADMIN.'/html/admin_datarelate_randomset.htm');
	exit();	
}
elseif($action=="fileperms")
{
	include(duomi_ADMIN.'/html/admin_datarelate_fileperms.htm');
	exit();	
}
elseif($action=="repeat")
{
	include(duomi_ADMIN.'/html/admin_reapeat.htm');
	exit();	
}
elseif($action=="checkpic")
{
	include(duomi_ADMIN.'/html/admin_datarelate_checkpic.htm');
	exit();	
}
elseif($action=="repairplaydata")
{
	...
	if($do!=true)
	{
	include(duomi_ADMIN.'/html/admin_repairplaydata.htm');
	exit();	
	}
	else 
	{
    $pagesize = 500;
	$row=$dsql->GetOne("select count(*) as dd from duomi_playdata");
	if(is_array($row))
	{
		$num = $row['dd'];
		if($num==0){
		echo "<div class=\"container\" id=\"cpcontainer\">没有数据</div>";
			...
			$sql="select v_id,tid,body from duomi_playdata limit $page $pagesize";
			$dsql->SetQuery($sql);
			...
			while($row = $dsql->GetArray('repairplaydata'))
				{
					...
					if(!empty($splay))
					{
						...
						if($change)
						{
							$rcount++;
							$fined=true;
							$dsql->ExecNoneQuery("update duomi_playdata set body= '".$splay."' where v_id=".$row['v_id']);
...
elseif($action=="dorandomset"){
	$pagesize = 30;
	$sql = " select v_id,v_name,v_hit from `duomi_data`";
	$dsql->SetQuery($sql);
	...
	$sql = "select v_id,v_name,v_hit from `duomi_data` order by v_id ASC limit $page $pagesize";
	$dsql->SetQuery($sql);
	...
	{
		$hit=rand(0, $maxHit);
		$upSql = "update `duomi_data` set v_hit=".$hit." where v_id =".$row[v_id];
		$dsql->ExecNoneQuery($upSql);
...
elseif($action=="delByFrom")
{
	...
	$page = isset($page) ? intval($page) : 1;
	if($page==0) $page=1;
	$csqlStr="select count(*) as dd from `duomi_playdata`".$wherestr;
	...
	$sql="select v_id,body from duomi_playdata ".$wherestr." limit 0,$numPerPage";
	$dsql->SetQuery($sql);
	...
	while($row=$dsql->GetObject('delByFrom'))
	{
		...
		$dsql->ExecuteNoneQuery("update duomi_playdata set body='$playdata' where v_id=".$row->v_id);
