...
function template_ext($template)
{
	...
	*/
	...
	/*
	 ...
	 {/if}				=>	<?php } ?>
	 */
	$template = preg_replace ( "/\{if\s+(.+?)\}\s*/i", "<?php if(\\1) { ?>", $template );
	$template = preg_replace ( "/\{else\}\s*/i", "<?php } else { ?>", $template );
	$template = preg_replace ( "/\{elseif\s+(.+?)\}\s*/i", "<?php } elseif (\\1) { ?>", $template );
	$template = preg_replace ( "/\{\/if\}\s*/i", "<?php } ?>", $template );
	//for 标签
	/*
	 {for $i=0;$i<10;$i++}	=>	<?php for($i=0;$i<10;$i++) { ?>
	 {/for}					=>	<?php } ?>
	 */
	$template = preg_replace("/\{for\s+(.+?)\}\s*/i","<?php for(\\1) { ?>",$template);
	$template = preg_replace("/\{\/for\}\s*/i","<?php } ?>",$template);
	...
	/*
	 ...
	 {/loop}					=>	<?php $n++;}unset($n) ?>
	 */
	$template = preg_replace ( "/\{loop\s+(\S+)\s+(\S+)\}\s*/i", "<?php \$n=1;if(is_array(\\1)) foreach(\\1 AS \\2) { ?>", $template );
	$template = preg_replace ( "/\{loop\s+(\S+)\s+(\S+)\s+(\S+)\}\s*/i", "<?php \$n=1; if(is_array(\\1)) foreach(\\1 AS \\2 => \\3) { ?>", $template );
	$template = preg_replace ( "/\{\/loop\}\s*/i", "<?php \$n++;}unset(\$n); ?>", $template );
